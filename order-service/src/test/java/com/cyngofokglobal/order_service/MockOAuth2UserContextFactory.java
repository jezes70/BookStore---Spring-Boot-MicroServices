    ...
    public SecurityContext createSecurityContext(WithMockOAuth2User withUser) {
        ...
        for (String role : withUser.role()) {
            if (role.startsWith("ROLE_")) {
                throw new IllegalArgumentException("role cannot start with ROLE_ Get " + role);
            }
        ...
        Jwt jwt = new Jwt("mock-jwt-token", Instant.now(), Instant.now().plusSeconds(400), headers, claims);
