const BOOKSHOP_STATE_KEY = "BOOKSHOP_STATE";

const getCart = function() {
    let cart = localStorage.getItem(BOOKSHOP_STATE_KEY)
    if (!cart) {
        cart = JSON.stringify({items:[], totalAmount:0 });
        localStorage.setItem(BOOKSHOP_STATE_KEY, cart)
    }
...
const addProductToCart = function(product) {
    ...
    localStorage.setItem(BOOKSHOP_STATE_KEY, JSON.stringify(cart));
    updateCartItemCount();
}
const updateProductQuantity = function(code, quantity) {
    ...
    if(quantity < 1) {
        ...
        if (cartItem) {
            ...
            console.log("Product code is already in Cart, ignoring")
        }
    }
    localStorage.setItem(BOOKSHOP_STATE_KEY, JSON.stringify(cart));
    updateCartItemCount();
...
const deleteCart = function() {
    localStorage.removeItem(BOOKSHOP_STATE_KEY)
    updateCartItemCount();
}
